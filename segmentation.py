...
def searchOptimalCuttingPoint_1(rgb, res_map, start, width_boundingbox, interval_range):
    t0 = time.time()
    #
    # for x in xrange(10):
    #     res_map = np.vstack((res_map, res_map[-1]))
    ...
    for zero_add in range(start, start+50, 3):
        ...
        for i in range(-8, int(width_boundingbox/1)-8):
            for refine in range(refine_s, int(width_boundingbox/2+3)):
                ...
                p6 = p5 + width_boundingbox + refine
                p7 = p6 + width_boundingbox + refine
                p8 = p7 + width_boundingbox + refine
                if p8 >= length:
                    continue
                score = res_map[p1][2]*3 -(res_map[p3][1]+res_map[p4][1]+res_map[p5][1]+res_map[p6][1]+res_map[p7][1]+res_map[p8][1])+8
                # print score
                score_list.append([score, [p1, p2, p3, p4, p5, p6, p7, p8]])
                p += 1
    print(p)
...
def refineCrop(sections, width=16):
    ...
    for section in sections:
        ...
        contours, hierarchy = cv2.findContours(binary_niblack, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) # imagex, 
        boxs = []
        ...
        # cv2.imshow("section"+str(i), section)
        # cv2.waitKey(0)
        i += 1
...
def slidingWindowsEval_1(image):
    windows_size = 16
    stride = 1
    height = image.shape[0]
    t0 = time.time()
    ...
    if len(interval) > 3:
        ...
        return []
    pin = np.array(pin)
    res = searchOptimalCuttingPoint_1(image, pin, 0, mid, 3)
